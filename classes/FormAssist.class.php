...
class FormAssist
{
	...
	public function checkBoxListColors($name,$attributes,$options,$vertical = true)
	{
		$this->_str="";
		if(isset($this->_fields[$name]))
		{
			...
			if(is_array($options))
			{
				...
				foreach($options as $label=>$value)
				{
					if(is_array($this->_fields[$name]))
					{
						if(in_array($value,$this->_fields[$name]))
						{
							$this->_str.="<div><label><input type='checkBox' name='$name"."[]' ".$this->_attributeString($attributes). " value='$value' checked='checked' />$label</label><div style='background-color:$value; width:20px;min-height:25px;float:left;'></div></div>$verticalStr";
						}
						else
						{
							$this->_str.="<div><label><input type='checkbox' name='$name"."[]".$this->_attributeString($attributes). " value='$value' />$label</label><div style='background-color:$value; width:20px;min-height:25px;float:left;'></div></div>$verticalStr";
						}
					}
					else
					{
